...
const Nav = ({ nav }) => {
    ...
    return (
        <nav className={'w-full  h-14 md:h-fit ' + (fixed?'fixed top-0 right-0 bg-sky-200 z-50s':'absolute top-0 right-0 ')}>
            <div className='relative mx-auto w-full md:w-fit'>
                <ul className='hidden lg:flex py-4 z-50 '>
                    {nav.map((item, index) => {
                        return (
                            <li key={index} className={(fixed?'inline-block px-6 py-3 mr-3 font-bold text-center uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-sky-400 to-lime-400 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-110 hover:rotate-2 hover:bg-pink-700 hover:text-white hover:shadow-lg active:opacity-85 ': 'inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle ') + (path.slice(0,-1)===item.href ? "text-white": "text-gray-600")}>
